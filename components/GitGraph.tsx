...
const GitGraph: React.FC<GitGraphProps> = ({ activeProject }) => {
  ...
  return (
    <div 
      className="p-8 bg-white border-4 border-black brutal-shadow overflow-x-hidden relative min-h-[600px]"
    ...
    >
      <div className="absolute top-4 right-4 text-xs font-black rotate-90 origin-right tracking-tighter opacity-10 select-none">
        REMOTE_LOG_{activeProject.title}_V1
      </div>
      
      <div className="mb-12 space-y-2">
        <h2 className="text-4xl font-black border-b-8 border-black pb-2 inline-block italic uppercase">Git_Flow_Graph</h2>
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-black uppercase bg-black text-white px-2">CURRENT_BRANCH:</span>
          <span className="text-[10px] font-mono font-bold italic">origin/refs/heads/main</span>
          {isSyncing && (
            <span className="text-[10px] text-green-600 animate-pulse font-black uppercase ml-4">[SYNCING_REMOTE...]</span>
          )}
        </div>
      </div>
      
      <div className={`relative transition-opacity duration-300 ${isSyncing ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
        {/* The backbone line */}
          ...
          {currentHistory.map((node) => {
            ...
            return (
              ...
              >
                ...
                {/* Content info */}
                <div className={`
                  border-l-4 border-black pl-4 py-1 transition-transform group-hover:translate-x-2 cursor-pointer flex-1
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="mt-4 flex gap-4">
                ...
                <button 
                  ...
                  onClick={() => window.alert('DEBUG: Raw binary diff not available.')}
                >
                  VIEW_RAW_DIFF
                </button>
              </div>
      ...
      <div className="mt-12 text-[10px] font-mono opacity-30 select-none">
        {`[system] node_count: ${currentHistory.length}`}
        <br />
        {`[system] target: ${activeProject.title}`}
        <br />
